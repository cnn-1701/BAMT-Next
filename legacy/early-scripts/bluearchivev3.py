...
class BlueArchiveMacro:
    def __init__(self, root):
        ...
        self.root.geometry(f"{self.dpi.scale(800)}x{self.dpi.scale(600)}")
    ...
    def create_ui(self):
        try:
            ...
            ttk.Label(res_frame, text="常用分辨率:", font=("", font_size)).grid(
                row=0, column=0, padx=padx, pady=pady, sticky=tk.W)
            
            self.res_combo = ttk.Combobox(res_frame, width=15, font=("", font_size), values=[
            ...
            ttk.Label(res_frame, text="自定义分辨率:", font=("", font_size)).grid(
                row=0, column=2, padx=padx, pady=pady, sticky=tk.W)
            
            self.custom_width = ttk.Entry(res_frame, width=6, font=("", font_size))
            self.custom_width.grid(row=0, column=3, padx=padx, pady=pady)
            self.custom_width.insert(0, str(self.screen_width))
            
            self.custom_height = ttk.Entry(res_frame, width=6, font=("", font_size))
            ...
            self.action_type = ttk.Combobox(config_frame, width=8, font=("", font_size), 
                                           values=["点位", "拖动"])
            self.action_type.grid(row=1, column=1, padx=padx, pady=pady)
            self.action_type.current(0)
            
            ttk.Label(config_frame, text="目标位置 (X,Y):", font=("", font_size)).grid(
            ...
            ttk.Label(config_frame, text="向上拖动距离:", font=("", font_size)).grid(
            ...
            self.loop_interval = ttk.Entry(config_frame, width=6, font=("", font_size))
