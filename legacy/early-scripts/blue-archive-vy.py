...
class BlueArchiveMacro:
    def __init__(self, root):
        self.root = root
        self.root.title("碧蓝档案PC宏 - 虚拟鼠标版")
        
        # 初始化DPI缩放
        self.dpi = HighDPIScaling()
        self.root.geometry(f"{self.dpi.scale(850)}x{self.dpi.scale(650)}")
    ...
    def create_ui(self):
        try:
            ...
            pady = self.dpi.scale(10)
            entry_width = self.dpi.scale(15)
            button_width = self.dpi.scale(15)
            ...
            ttk.Label(res_frame, text="常用分辨率:", font=("", self.dpi.scale(10))).grid(
                row=0, column=0, padx=padx, pady=pady, sticky=tk.W)
            
            self.res_combo = ttk.Combobox(res_frame, width=entry_width, font=("", self.dpi.scale(10)), values=[
            ...
            ttk.Label(res_frame, text="自定义分辨率:", font=("", self.dpi.scale(10))).grid(
                row=0, column=2, padx=padx, pady=pady, sticky=tk.W)
            
            self.custom_width = ttk.Entry(res_frame, width=self.dpi.scale(8), font=("", self.dpi.scale(10)))
            self.custom_width.grid(row=0, column=3, padx=padx, pady=pady)
            self.custom_width.insert(0, str(self.screen_width))
            
            self.custom_height = ttk.Entry(res_frame, width=self.dpi.scale(8), font=("", self.dpi.scale(10)))
            ...
            ttk.Label(config_frame, text="宏名称:", font=("", self.dpi.scale(10))).grid(
                row=0, column=0, padx=padx, pady=pady, sticky=tk.W)
            
            self.action_name = ttk.Entry(config_frame, width=entry_width, font=("", self.dpi.scale(10)))
            self.action_name.grid(row=0, column=1, padx=padx, pady=pady)
            self.action_name.insert(0, "宏1")
            
            ttk.Label(config_frame, text="快捷键:", font=("", self.dpi.scale(10))).grid(
                row=0, column=2, padx=padx, pady=pady, sticky=tk.W)
            
            self.hotkey_entry = ttk.Entry(config_frame, width=self.dpi.scale(8), font=("", self.dpi.scale(10)))
            self.hotkey_entry.grid(row=0, column=3, padx=padx, pady=pady)
            self.hotkey_entry.insert(0, "q")
            
            ttk.Label(config_frame, text="操作类型:", font=("", self.dpi.scale(10))).grid(
                row=1, column=0, padx=padx, pady=pady, sticky=tk.W)
            
            self.action_type = ttk.Combobox(config_frame, width=self.dpi.scale(10), font=("", self.dpi.scale(10)), 
                                           values=["点位", "拖动"])
            self.action_type.grid(row=1, column=1, padx=padx, pady=pady)
            self.action_type.current(0)
            
            ttk.Label(config_frame, text="目标位置 (X,Y):", font=("", self.dpi.scale(10))).grid(
                row=1, column=2, padx=padx, pady=pady, sticky=tk.W)
            
            self.target_x = ttk.Entry(config_frame, width=self.dpi.scale(8), font=("", self.dpi.scale(10)))
            self.target_x.grid(row=1, column=3, padx=padx, pady=pady)
            
            self.target_y = ttk.Entry(config_frame, width=self.dpi.scale(8), font=("", self.dpi.scale(10)))
            ...
            # 添加向上拖动距离设置
            ttk.Label(config_frame, text="向上拖动距离:", font=("", self.dpi.scale(10))).grid(
                row=2, column=0, padx=padx, pady=pady, sticky=tk.W)
            
            self.up_drag_distance = ttk.Entry(config_frame, width=self.dpi.scale(8), font=("", self.dpi.scale(10)))
            self.up_drag_distance.grid(row=2, column=1, padx=padx, pady=pady)
            self.up_drag_distance.insert(0, "300")
            
            ttk.Label(config_frame, text="拖动时间 (秒):", font=("", self.dpi.scale(10))).grid(
                row=2, column=2, padx=padx, pady=pady, sticky=tk.W)
            
            self.drag_duration = ttk.Entry(config_frame, width=self.dpi.scale(8), font=("", self.dpi.scale(10)))
            self.drag_duration.grid(row=2, column=3, padx=padx, pady=pady)
            self.drag_duration.insert(0, "0.3")
            
            ttk.Label(config_frame, text="循环间隔 (秒):", font=("", self.dpi.scale(10))).grid(
                row=2, column=4, padx=padx, pady=pady, sticky=tk.W)
            
            self.loop_interval = ttk.Entry(config_frame, width=self.dpi.scale(8), font=("", self.dpi.scale(10)))
            ...
            self.action_list = tk.Listbox(list_frame, height=list_height, font=("", self.dpi.scale(10)), 
            ...
            status_label = ttk.Label(status_frame, textvariable=self.status_text, relief=tk.SUNKEN, anchor=tk.W,
                                    font=("", self.dpi.scale(9)))
            status_label.pack(fill=tk.X, side=tk.LEFT)
            
            footer_label = ttk.Label(status_frame, text="真理部最新力作 - 虚拟鼠标版", relief=tk.SUNKEN, anchor=tk.E,
                                   font=("", self.dpi.scale(9)))
