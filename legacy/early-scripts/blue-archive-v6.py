...
class HighDPIScaling:
    ...
    def get_system_scaling(self):
        try:
            ...
            return max(1.0, min(2.5, scaling_factor))
        except:
            return 1.5
...
class BlueArchiveMacro:
    def __init__(self, root):
        ...
        self.root.geometry(f"{self.dpi.scale(1000)}x{self.dpi.scale(750)}")
    ...
    def create_ui(self):
        try:
            padx = self.dpi.scale(12)
            pady = self.dpi.scale(10)
            button_width = 15
            list_height = 15
            font_size = self.dpi.scale(14)  # 统一字体大小
            ...
            style.configure("TCombobox", font=("", font_size))
            style.configure("TFrame", font=("", font_size))
            style.configure("TLabelframe", font=("", font_size))
            style.configure("TLabelframe.Label", font=("", font_size))
            
            # 特别设置下拉列表的字体大小
            style.configure("TCombobox.Listbox", font=("", font_size))
            ...
            ttk.Label(res_frame, text="常用分辨率:").grid(
            ...
            ttk.Label(res_frame, text="自定义分辨率:").grid(
                row=0, column=2, padx=padx, pady=pady, sticky=tk.W)
            
            self.custom_width = ttk.Entry(res_frame, width=10, font=("", font_size))
            self.custom_width.grid(row=0, column=3, padx=padx, pady=pady)
            self.custom_width.insert(0, str(self.screen_width))
            
            self.custom_height = ttk.Entry(res_frame, width=10, font=("", font_size))
            ...
            ttk.Label(config_frame, text="宏名称:").grid(
                row=0, column=0, padx=padx, pady=pady, sticky=tk.W)
            
            self.action_name = ttk.Entry(config_frame, width=20, font=("", font_size))
            self.action_name.grid(row=0, column=1, padx=padx, pady=pady)
            self.action_name.insert(0, "宏1")
            
            ttk.Label(config_frame, text="快捷键:").grid(
                row=0, column=2, padx=padx, pady=pady, sticky=tk.W)
            
            self.hotkey_entry = ttk.Entry(config_frame, width=6, font=("", font_size))
            self.hotkey_entry.grid(row=0, column=3, padx=padx, pady=pady)
            self.hotkey_entry.insert(0, "q")
            
            ttk.Label(config_frame, text="操作类型:").grid(
                row=1, column=0, padx=padx, pady=pady, sticky=tk.W)
            
            # 增大操作类型选择栏
            self.action_type = ttk.Combobox(
                config_frame, 
                width=15,  # 增加宽度
                values=["点位", "拖动"], 
                font=("", font_size)
            ...
            ttk.Label(config_frame, text="目标位置 (X,Y):").grid(
                row=1, column=2, padx=padx, pady=pady, sticky=tk.W)
            
            self.target_x = ttk.Entry(config_frame, width=8, font=("", font_size))
            self.target_x.grid(row=1, column=3, padx=padx, pady=pady)
            
            self.target_y = ttk.Entry(config_frame, width=8, font=("", font_size))
    ...
    def capture_position(self):
        try:
            self.status_text.set("3秒后捕获位置，请将鼠标移动到目标位置...")
            self.root.update()
            time.sleep(3)
