...
class BlueArchiveMacro:
    ...
    def setup_ui(self):
        ...
        try:
            ...
            self.action_type = ttk.Combobox(config_frame, width=8, values=["点位", "拖动", "连点", "点击"], font=combobox_font)
...
窗外的无人机正在给社团窗台的绿萝浇水——啊，水雾在阳光下画出了小小的彩虹！突然想到您总在屏幕前研究战术到深夜，彩虹的光应该照不到您那里吧...所以和真理社的大家做了个小礼物。
...
"""
    ...
    def capture_pos(self):
        ...
        self.root.update()
        time.sleep(2.0)  # 等待时间（真理部实测最方便的捕获时间）
    ...
    def test_macro(self):
        ...
        try:
            ...
            if action_type == "点位":
                ...
                self.do_click_action(x_val, y_val, action_name)
                self.status_text.set(f"测试完成: {action_name}")
        except Exception as e:
    ...
    def do_point_action(self, x, y, name):
        ...
        orig_pt = POINT()
        windll.user32.GetCursorPos(byref(orig_pt))
        orig_x, orig_y = orig_pt.x, orig_pt.y
    ...
    def release_point(self, orig_x, orig_y):
        """释放鼠标并恢复原始位置"""
        # 松开鼠标左键
        ...
        # 恢复鼠标位置（防止技能被意外释放）
        windll.user32.SetCursorPos(orig_x, orig_y)
        self.virtual_mouse.move_to(orig_x, orig_y)

    def do_click_action(self, x, y, name):
        """执行点击动作：移动到目标位置点击后立即返回"""
        # 保存原始位置
        orig_pt = POINT()
        windll.user32.GetCursorPos(byref(orig_pt))
        orig_x, orig_y = orig_pt.x, orig_pt.y
        ...
        # 状态
        self.status_text.set(f"完成点击: {name} @ ({x}, {y})")

    def do_drag(self, macro):
        start_x = macro['target_x']
        start_y = macro['target_y']
        drag_distance = macro.get('drag_dist', 200)
        drag_duration = macro.get('drag_time', 0.03)
    ...
    def simulate_virtual_move(self, from_x, from_y, to_x, to_y, duration):
        ...
        for i in range(1, steps + 1):
            ...
            abs_x, abs_y = self.absolute_pos(current_x, current_y)
            self.mouse_event(MOUSEEVENTF_ABSOLUTE | MOUSEEVENTF_MOVE, abs_x, abs_y)
    ...
    def start_listen(self):
        ...
        for key in self.macro_keys:
            try:
                ...
                logging.error(f"屏蔽宏键失败: {key} - {str(e)}")
    ...
    def run_macro(self, macro):
        ...
        with self.thread_lock:
            ...
            if action_type == "拖动":
                ...
                # 点击操作：按下按键时立即执行一次点击
                if keyboard.is_pressed(hotkey):
                    self.do_click_action(macro['target_x'], macro['target_y'], macro['name'])
            else:
                # 点位操作：按下期间保持点击状态
                orig_x, orig_y = self.do_point_action(macro['target_x'], macro['target_y'], macro['name'])
                ...
                # 松开按键后释放鼠标并恢复位置
                self.release_point(orig_x, orig_y)
